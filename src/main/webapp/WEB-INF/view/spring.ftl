...
<input type="${fieldType}" id="${status.expression}" name="${status.expression}"
       value="<#if fieldType!="password">${stringStatusValue}</#if>" ${attributes}
    <@closeTag/>
</#macro>
...
<#macro formSingleSelect path options attributes="">
    <@bind path/>
<select id="${status.expression}" class="appInputText appRoundedCorner appBlock appIconBrownBottom2"
 ...
 * formMultiSelect
 *
 * Show a listbox of options allowing the userRds to make 0 or more choices from
 * the list of options.
 *
 * @param path the name of the field to bind to
 * @param options a map (value=label) of all the available options
...
<#macro showTutorialErrors>
    <#list status.errorMessages as error>
    <div class="appError">${error}</div>
    </#list>
</#macro>
 ...
 * This function is used internally but can be accessed by userRds code if required.
 *
